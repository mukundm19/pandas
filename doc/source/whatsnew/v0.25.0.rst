...
- Bug in :func: _cython_agg_general() causing a fallback when skipna flag was used. Fallback no longer is caused by skipna flag but still occurs when strings are passed in but this stems from the _aggregate() call in ops.py (:issue:`20824`)
